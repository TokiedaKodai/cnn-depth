...
for idx in tqdm(data_idx_range):
    ...
    cv2.imwrite(data_dir + 'rec_ajusted/{:05d}.bmp'.format(idx), img_rec_ajust)
    xyz_rec = depth_tools.convert_depth_to_coords(depth_rec_ajust, cam_params)
    depth_tools.dump_ply(data_dir + '/ply_rec_ajusted/{:05d}.ply'.format(idx), xyz_rec.reshape(-1, 3).tolist())
